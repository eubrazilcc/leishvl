...
import static io.leishvl.core.util.MimeUtils.mimeType;
import static io.leishvl.storage.Filter.FilterType.FILTER_EXISTS;
import static io.leishvl.storage.mongodb.MongoCollectionConfigurer.hash2bucket;
...
import io.leishvl.core.concurrent.CancellableTask;
...
import io.leishvl.storage.base.LeishvlFile;
import io.leishvl.storage.base.LeishvlFiles;
import io.leishvl.storage.base.Metadata;
import io.leishvl.storage.base.Metadata.OpenAccess;
import io.leishvl.storage.mongodb.MongoCollectionStats;
import io.leishvl.storage.mongodb.cache.CachedGridFSFile;
import io.leishvl.storage.mongodb.cache.GridFSFilePersistingCache;
...
public class MongoFileClient implements MongoClient2 {
	...
	public <T extends LeishvlFile> ListenableFuture<Void> saveFile(final T obj, final File srcFile, final boolean override) {
	...
	private <T extends LeishvlFile> ListenableFuture<Void> deleteAndSave(final T obj, final File srcFile) {
	...
	private <T extends LeishvlFile> ListenableFuture<Void> saveFile(final T obj, final File srcFile) {		
	...
	public <T extends LeishvlFile> ListenableFuture<LeishvlFile> fetchFile(final LeishvlFile obj, final Class<T> type) {
		checkArgument(obj != null && obj.getClass().isAssignableFrom(type), "Uninitialized or invalid object type");
		final CancellableTask<LeishvlFile> task = new MongoTask<LeishvlFile>(ListenableFutureTask.create(new Callable<LeishvlFile>() {
			@Override
			public LeishvlFile call() throws Exception {
				final Connection conn = Connection.newBucket(client(), obj);		
				final GridFS gridFS = (conn.bucket != null ? new GridFS(conn.db, conn.bucket) : new GridFS(conn.db));
				final GridFSDBFile gfsFile = gridFS.findOne(conn.filename);
				if (gfsFile == null) throw new LeishvlObjectNotFoundException("Object not found");
				T result = null;
				try {
	 ...
	 * @return
	 */
	public <T extends LeishvlFile> ListenableFuture<Boolean> fileExists(final T obj) {
	...
	public <T extends LeishvlFile> ListenableFuture<List<T>> fetchFiles(final LeishvlFiles<T> files, final Class<T> type, final int start, final int size, 
	 ...
	 */
	public <T extends LeishvlFile> ListenableFuture<Boolean> removeFile(final T obj) {
		final CancellableTask<Boolean> task = new MongoTask<Boolean>(ListenableFutureTask.create(new Callable<Boolean>() {
			@Override
			public Boolean call() throws Exception {
				final Connection conn = Connection.newBucket(client(), obj);
	...
	public <T extends LeishvlFile> ListenableFuture<Void> updateMetadata(final T obj) {
		final CancellableTask<Void> task = new MongoTask<Void>(ListenableFutureTask.create(new Callable<Void>() {
			@Override
			public Void call() throws Exception {
				final Connection conn = Connection.newBucket(client(), obj);
	...
	public <T extends LeishvlFile> ListenableFuture<Void> createOpenAccessLink(final T obj) {
	...
	public <T extends LeishvlFile> ListenableFuture<Void> removeOpenAccessLink(final T obj) {
		checkArgument(obj != null && obj.getMetadata() != null, "Uninitialized or invalid file object");
		obj.getMetadata().setOpenAccess(null);
		return updateMetadata(obj);
	}

	public <T extends LeishvlFile> ListenableFuture<List<T>> findOpenAccess(final LeishvlFiles<T> files, final Class<T> type, final int start, 
	...
	public <T extends LeishvlFile> ListenableFuture<LeishvlFile> fetchOpenAccessFile(final LeishvlFile obj, final Class<T> type) {
		checkArgument(obj != null && obj.getClass().isAssignableFrom(type), "Uninitialized or invalid object type");
		final CancellableTask<LeishvlFile> task = new MongoTask<LeishvlFile>(ListenableFutureTask.create(new Callable<LeishvlFile>() {
			@Override
			public LeishvlFile call() throws Exception {
	...
	public <T extends LeishvlFile> ListenableFuture<List<String>> typeaheadFile(final LeishvlFiles<T> files, final String query, final int size) {
	...
	public <T extends LeishvlFile> ListenableFuture<Long> totalCount(final LeishvlFiles<T> files) {
	...
	public <T extends LeishvlFile> ListenableFuture<MongoCollectionStats> statsFiles(final LeishvlFiles<T> files) {
	...
	private static <T extends LeishvlFile> T parseGridFSDBFile(final GridFSDBFile gfsFile, final Class<T> type) throws IOException {
	...
	private static class Connection {
		...
		public static <T extends LeishvlFile> Connection newBucket(final MongoClient client, final T obj) {
		...
		public static <T extends LeishvlFile> Connection newBucket(final MongoClient client, final LeishvlFiles<T> files) {
			final DB db = client.getDB(CONFIG_MANAGER.getDbName());
			final String bucket = getBucket(files).orNull();
			return new Connection(db, bucket, null, null);
		}
		public static <T extends LeishvlFile> Connection newBucketOpenAccess(final MongoClient client, final T obj) {
			final DB db = client.getDB(CONFIG_MANAGER.getDbName());
		...
		private static <T extends LeishvlFile> Optional<Integer> getBucketNumber(final T obj) {
			checkArgument(obj != null && obj.getMetadata() != null, "Uninitialized or invalid file object");
			Integer bucket = null;
		...
		private static <T extends LeishvlFile> Optional<String> getBucket(final T obj) {
			checkArgument(obj != null && obj.getMetadata() != null, "Uninitialized or invalid file object");
		...
		private static <T extends LeishvlFile> Optional<String> getBucket(final LeishvlFiles<T> files) {			
		...
		private static <T extends LeishvlFile> String getBucketFilename(final T obj) {
			return getBucketFilename(obj, true);
		}
		private static <T extends LeishvlFile> String getBucketFilename(final T obj, final boolean addNamespace) {
			checkArgument(obj != null && obj.getMetadata() != null, "Uninitialized or invalid file object");
		...
		private static <T extends LeishvlFile> String getOpenAccessBucket(final T obj) {
		...
		private static <T extends LeishvlFile> String getOpenAccessSecret(final T obj) {
			checkArgument(obj != null && obj.getMetadata() != null && obj.getMetadata().getOpenAccess() != null, "Uninitialized or invalid file object");
			String secret = null;
