...
public class GridFSFilePersistingCache {
	...
	public CachedGridFSFile getIfPresent(final String filename) {
		return CACHE.getIfPresent(toKey(filename));
	}

	public CachedGridFSFile put(final GridFSDBFile gfsFile) throws IOException {		
		checkArgument(gfsFile != null && isNotBlank(gfsFile.getFilename()) && isNotBlank(gfsFile.getMD5()), "Uninitialized or invalid file");
		File outfile = null;
		try {
			...
			CACHE.put(toKey(gfsFile.getFilename()), cachedFile);
			return cachedFile;
		} catch (Exception e) {
	...
	public CachedGridFSFile update(final GridFSDBFile gfsFile) throws IOException {
		checkArgument(gfsFile != null && isNotBlank(gfsFile.getFilename()) && isNotBlank(gfsFile.getMD5()), "Uninitialized or invalid file");
		invalidate(gfsFile.getFilename());
		return put(gfsFile);
	}

	public void invalidate(final String filename) {
		CACHE.invalidate(toKey(filename));
	}
	...
	private static String toKey(final String filename) {
		String filename2 = null;
		checkArgument(isNotBlank(filename2 = trimToNull(filename)), "Uninitialized or invalid filename");
		return filename2;
	}
