...
import static io.leishvl.storage.base.SaveOptions.SAVE_OVERRIDING;
...
import io.leishvl.storage.mongodb.MongoFilesConfigurer;
 ...
 */
public abstract class LeishvlFile extends LeishvlBaseFile implements Linkable {
	...
	public LeishvlFile(final MongoFilesConfigurer configurer, final Logger logger) {
	...
	public boolean equals(final Object obj) {
		if (obj == null || !(obj instanceof LeishvlFile)) {
			return false;
		}
		final LeishvlFile other = LeishvlFile.class.cast(obj);
		return super.equals((LeishvlBaseFile)other);
	}
	...
	public ListenableFuture<Void> fetch() {
		...
		final LeishvlFile __file = this;
		final ListenableFuture<LeishvlFile> fetchFuture = MONGODB_CONN.fsClient().fetchFile(this, this.getClass());
		addCallback(fetchFuture, new FutureCallback<LeishvlFile>() {
			@Override
			public void onSuccess(final LeishvlFile result) {
				try {
	...
	public ListenableFuture<Void> fetchOpenAccess() {
		final SettableFuture<Void> future = SettableFuture.create();
		final LeishvlFile __file = this;
		final ListenableFuture<LeishvlFile> fetchFuture = MONGODB_CONN.fsClient().fetchOpenAccessFile(this, this.getClass());
		addCallback(fetchFuture, new FutureCallback<LeishvlFile>() {
			@Override
			public void onSuccess(final LeishvlFile result) {
				try {
	...
	public static void copyProperties(final LeishvlFile orig, final LeishvlFile dest) throws IllegalAccessException, InvocationTargetException {
	...
	public abstract static class Builder<T extends LeishvlFile, B extends Builder<T, B>> {
